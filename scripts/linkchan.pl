...
%IRSSI =
(
    authors     => 'Foxmask <odemah@phpfr.org> & Marcin \'Qrczak\' Kowalczyk',
    contact     => 'qrczak@knm.org.pl',
    name        => 'LinkChan',
    description => 'Link several channels on serveral networks',
    license     => 'GNU GPL',
    url         => 'http://qrnik.knm.org.pl/~qrczak/irc/linkchan.pl',
);
...
#        my $perm = $nk->{op} ? '@' : $nk->{voice} ? '+' : ' ';
        message $context->{chan2}, "<$name> $msg";
#    }
};
...
Irssi::signal_add "message topic", sub
{
    ...
    {
        special_message $context->{chan2},
          "Topic unset by $name on $context->{channel1}";
        $context->{server2}->command("topic -delete $context->{channel2}");
    }
    else
    {
        special_message $context->{chan2},
          "$name changed the topic of $context->{channel1} to: $topic";
